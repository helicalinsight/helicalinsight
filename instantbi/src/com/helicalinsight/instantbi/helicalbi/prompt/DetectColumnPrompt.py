...
10. Populate selectColumnName with only the columns that belong in the SELECT clause (the projected output). Do not put WHERE / HAVING / JOIN-key-only columns in selectColumnName; those still belong in columnName.
11. Never use * or COUNT(*). Always pick explicit physical column names (table.column). For counts or "how many" questions, pick a real column (prefer unique identifier / primary key) so SQL can use COUNT(column) or COUNT(DISTINCT column). Do not put * in columnName or selectColumnName.
"""
