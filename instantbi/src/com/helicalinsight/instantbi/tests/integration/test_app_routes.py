...
class TestInteractive:
    ...
    def test_rm_cols_in_filter_defaults_true_and_query_param_disables(
        self, app_module, flask_client, session_auth, patch_graphs
    ):
        ...
        with ExitStack() as stack:
            ...
            resp = flask_client.post("/interactive", json=payload)
        assert resp.status_code == 200
        assert form_default.call_args.kwargs["rm_cols_in_filter"] is True

        with ExitStack() as stack:
            for ctx in _patch_interactive_pipeline(app_module, helper_mock):
                stack.enter_context(ctx)
            form_off = stack.enter_context(
                patch(
                    "helicalbi.viz.viz_model_fill._try_sql_to_form_data",
                    return_value={"columns": [{"alias": "a"}]},
                )
            ...
            resp = flask_client.post(
                "/interactive?rm_cols_in_filter=false", json=payload
            )
        assert resp.status_code == 200
        assert form_off.call_args.kwargs["rm_cols_in_filter"] is False
...
class TestSqlToReportModel:
    def test_returns_report_model(self, flask_client, session_auth):
        ...
        assert report_model["viz_model"]["data"]["rows"] == ["region"]
        assert report_model["viz_model"]["data"]["columns"] == ["amount"]
