...
class TestFetchServiceApi:
    ...
    def test_cache_key_varies_by_username_and_orgname(self, session_cookie):
        ...
        with patch(
            ...
            set_api_cache_identity("alice", "acme", user_id=1, org_id=5)
            fetch_service_api(session_cookie=session_cookie, service_json=service_json)
            set_api_cache_identity("bob", "acme", user_id=2, org_id=5)
            fetch_service_api(session_cookie=session_cookie, service_json=service_json)

        assert session.post.call_count == 2

    def test_cache_key_varies_by_org_id(self, session_cookie):
        session = self._session_with_status(200, {"status": 1, "response": {"ok": True}})
        service_json = {
            "service": "get",
            "formData": '{"metadataFileName":"meta.json"}',
        ...
        with patch(
            ...
            set_api_cache_identity("alice", "acme", user_id=1, org_id=1)
            fetch_service_api(session_cookie=session_cookie, service_json=service_json)
            set_api_cache_identity("alice", "acme", user_id=1, org_id=2)
            fetch_service_api(session_cookie=session_cookie, service_json=service_json)
    ...
    def test_raises_service_api_error_on_status_zero(self, session_cookie):
        from helicalbi.api.HttpCallService import ServiceApiError

        session = self._session_with_status(
            200,
            {
                "status": 0,
                "response": {"message": "Error: The file missing.model doesn't exists."},
            },
        )
        with patch(
            "helicalbi.api.HttpCallService.requests.Session", return_value=session
        ):
            with pytest.raises(ServiceApiError, match="doesn't exists"):
                fetch_service_api(
                    session_cookie=session_cookie,
                    service_json={"service": "getAiAgent", "formData": "{}"},
                )

    def test_does_not_cache_status_zero_responses(self, session_cookie, monkeypatch):
        from helicalbi.api.HttpCallService import ServiceApiError
        from helicalbi.common import app_config
        ...
        session = self._session_with_status(
            200,
            {"status": 0, "response": {"message": "metadata unavailable"}},
        )
        service_json = {
            "service": "get",
            "formData": '{"metadataFileName":"meta.json"}',
        ...
        with patch(
            ...
            with pytest.raises(ServiceApiError, match="metadata unavailable"):
                fetch_service_api(session_cookie=session_cookie, service_json=service_json)
            with pytest.raises(ServiceApiError, match="metadata unavailable"):
                fetch_service_api(session_cookie=session_cookie, service_json=service_json)
    ...
    def test_execute_query_status_zero_returns_payload(self, session_cookie):
        session = self._session_with_status(
            200,
            {"status": 0, "response": "syntax error"},
        )
        with patch(
            "helicalbi.api.HttpCallService.requests.Session", return_value=session
        ):
            result = fetch_service_api(
                session_cookie=session_cookie,
                service_json={"service": "executeQuery", "formData": "{}"},
            )
        assert result == {"status": 0, "response": "syntax error"}
