...
class TestColumnResponse:
    ...
    def test_accepts_select_clause_columns(self):
        m = ColumnResponse(
            columnName=["travel_details.travel_cost", "employee.emp_id"],
            selectColumnName=["travel_details.travel_cost"],
            reason="x",
        )
        assert m.selectColumnName == ["travel_details.travel_cost"]
...
class TestOptionalPromptReason:
    ...
    def test_final_sql_prompt_skips_join_when_select_is_one_table(self):
        from helicalbi.prompt.FinalSqlPrompt import final_sql_prompt

        assert "every column in the SELECT clause belongs to the same table" in (
            final_sql_prompt
        )
    ...
    def test_sql_prompts_forbid_star_and_count_star(self):
        ...
        assert "Never use * or COUNT(*)" in detect_column_prompt_string
