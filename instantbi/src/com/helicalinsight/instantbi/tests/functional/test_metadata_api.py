...
class TestGetJsonDataMetadata:
    ...
    def test_raises_when_service_returns_status_zero(self, session_cookie):
        from helicalbi.api.HttpCallService import ServiceApiError

        with patch(
            "helicalbi.api.Metadata.fetch_service_api",
            return_value={"status": 0, "response": {"message": "Error: metadata missing"}},
        ):
            with pytest.raises(ServiceApiError, match="metadata missing"):
                get_json_data_metadata(session_cookie, "m", "/d")
