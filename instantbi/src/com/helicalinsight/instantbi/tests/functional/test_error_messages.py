...
class TestErrorMessages:
    ...
    def test_service_api_error_message_reads_response_message(self):
        assert (
            service_api_error_message(
                {"status": 0, "response": {"message": "Error: join failed"}}
            )
            == "join failed"
        )

    def test_service_api_error_message_reads_string_response(self):
        assert (
            service_api_error_message({"status": 0, "response": "syntax error"})
            == "syntax error"
        )
