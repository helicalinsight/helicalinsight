...
const App = ({ store }) => {
  const props = {
    urlObj: { dir: "sai_ganesh", file: "test.metadata", mode: "open" },
  };
  return (
...
describe("InstantBI visualisation", () => {
  ...
  test("Instant BI Direct Url metadata loading", async () => {
    ...
    // expect(getState().instantBI.metadata).toStrictEqual({
    //   location: "sai_ganesh",
    //   metadataFileName: "test.metadata",
    //   path: "sai_ganesh/test.metadata",
    //   title: "test",
    // });
  });
});
