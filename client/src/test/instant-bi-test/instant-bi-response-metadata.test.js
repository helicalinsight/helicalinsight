...
describe("InstantBIResponseMetadata", () => {
  ...
  it("should use singular labels when count is 1", () => {
    const { container } = render(
      <InstantBIResponseMetadata
        sqlDetails={{
          ...sampleSqlDetails,
          required_table: ["employee_details"],
          required_column: ["travel_details.destination"],
          required_cube_info: {
    ...
    const footerText = container.querySelector(".ibm-footer")?.textContent || "";
    expect(footerText).toMatch(/1\s+table\b/i);
    expect(footerText).toMatch(/1\s+column\b/i);
    expect(footerText).toMatch(/1\s+dimension\b/i);
    expect(footerText).toMatch(/1\s+metric\b/i);
    expect(footerText).not.toMatch(/1\s+tables\b/i);
  });
