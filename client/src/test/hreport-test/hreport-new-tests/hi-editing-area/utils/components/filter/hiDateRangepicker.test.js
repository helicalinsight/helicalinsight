  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <Provider store={store}>
        <HiDateRangePicker {...props} onChange={handleOnChange} />
      </Provider>
    </DndProvider>
  );
};

describe("Rendering HiDateRangePicker", () => {
  beforeAll(() => {
  ...
  test("HiDateRangePicker component", async () => {
    await waitFor(() => render(<App hiDate_picker={hiDate_picker} />));

    const comp = screen.queryByTestId(/Hi-report-hidate-rangepicker/i);
