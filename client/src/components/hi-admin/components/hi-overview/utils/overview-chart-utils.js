...
const formatChartValue = (datum) => {
  if (datum?.displayValue != null) return datum.displayValue;
  const num = Number(datum?.value);
  return Number.isFinite(num) ? num.toLocaleString() : String(datum?.value ?? "");
};
...
const getColumnConfig = ({ data, height = 200, color = OVERVIEW_CHART_COLORS.secondary }) => ({
  ...
  label: {
    position: "top",
    style: { fontSize: 11 },
    formatter: (datum) => formatChartValue(datum),
  },
  xAxis: {
