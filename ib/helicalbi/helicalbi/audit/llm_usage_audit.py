...
from helicalbi.common.auth import get_api_cache_username
    ...
    *,
    ...
    try:
        ...
        if not persisted:
            ...
            return
        logger.debug(
            "LLM usage audit persisted endpoint=%s user=%s tokens=%s",
            endpoint,
            username,
        ...
        logger.exception("LLM usage audit request failed endpoint=%s user=%s", endpoint, username)
...
def audit_llm_usage_async(
    *,
    ...
    resolved_username = (username or get_api_cache_username() or "").strip()
    if not resolved_session_cookie or not resolved_base_url or not resolved_username:
        logger.debug(
            "Skipping LLM usage audit due to missing session context endpoint=%s "
            "has_session=%s has_base_url=%s has_username=%s",
            endpoint,
