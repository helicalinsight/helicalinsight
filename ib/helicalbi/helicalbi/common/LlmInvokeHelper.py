    ...
    if isinstance(inputs, dict) and prompt is not None:
        if hasattr(prompt, "format_prompt"):
            try:
                ...
                logger.error(
                    "prompt.format_prompt failed; trying alternate formatting",
                    exc_info=True,
                )
        if hasattr(prompt, "format"):
            try:
                ...
                logger.error(
                    "prompt.format failed; falling back to json.dumps of inputs",
                    exc_info=True,
                )
...
def read_token_usage(state: dict) -> TokenUsage:
    """Read accumulated token usage from model state."""
...
def merge_token_usage(state: dict, usage: TokenUsage) -> None:
    """Add token usage from an LLM call into model state."""
...
def read_time_consumed(state: dict) -> TimeConsumed:
    """Read accumulated timing from model state."""
...
def merge_time_consumed(state: dict, consumed: TimeConsumed) -> None:
    """Add elapsed time from an LLM call into model state."""
...
def set_total_time_consumed(state: dict, total_seconds: float) -> None:
    """Record end-to-end request duration on model state."""
