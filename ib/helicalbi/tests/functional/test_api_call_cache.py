...
class TestApiCallCache:
    ...
    def test_clear_returns_removed_entry_count(self):
        cache_set('{"a": 1}', "alice", "acme", {"status": 1}, org_id=5)
        cache_set('{"b": 2}', "alice", "acme", {"status": 1}, org_id=5)
        assert clear_api_cache() == 2
        assert clear_api_cache() == 0

    def test_cache_key_varies_by_org_id(self):
        cache_set('{"a": 1}', "alice", "acme", {"status": 1, "response": {"org": 1}}, org_id=1)
        cache_set('{"a": 1}', "alice", "acme", {"status": 1, "response": {"org": 2}}, org_id=2)

        assert cache_get('{"a": 1}', "alice", "acme", org_id=1) == {
            "status": 1,
            "response": {"org": 1},
        }
        assert cache_get('{"a": 1}', "alice", "acme", org_id=2) == {
            "status": 1,
            "response": {"org": 2},
        }
        assert cache_get('{"a": 1}', "alice", "acme", org_id=None) is None
