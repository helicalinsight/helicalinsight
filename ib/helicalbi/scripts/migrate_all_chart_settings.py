...
def _pie(stem: str, *, donut: bool = False) -> list[str]:
    ...
    lines = [
        ...
        *FORMAT_HELPER,
        ...
        "    colorField: setting.dimensions.names[0],",
        "    radius: 0.8,",
    ]
    if donut:
        lines.extend(
...
def _heatmap() -> list[str]:
    return [
        ...
        "  const setting = ${setting};",
        *FORMAT_HELPER,
        "  const xDim = setting.dimensions.names[0];",
        "  const yDim = setting.dimensions.names[1];",
        "  const config = {",
        "    data,",
        "    xField: xDim,",
        "    yField: yDim,",
...
def _bubble() -> list[str]:
    return [
        ...
        *FORMAT_HELPER,
        ...
        "    size: [4, 30],",
        "    shape: 'circle',",
        "    ...(setting.dimensions.names[0] ? { colorField: setting.dimensions.names[0] } : {}),",
        "    ...(setting.color ? { color: setting.color } : {}),",
...
def _grid_table(stem: str, *, multi_dim: bool = False) -> list[str]:
    return [
        ...
        "  const dimensions = (setting.dimensions && setting.dimensions.names) || [];",
        "  const measures = setting.measures || [];",
        "  const preferred = [...dimensions, ...measures].filter(Boolean);",
        "  const allKeys = data.length > 0 ? Object.keys(data[0]) : [];",
        "  const keys = preferred.length",
        "    ? [...preferred, ...allKeys.filter((k) => !preferred.includes(k))]",
        "    : allKeys;",
        ...
        "      if (",
        "        cls &&",
        "        (cls.contains('ib-live-preview') ||",
        "          cls.contains('chart-wrapper__content') ||",
        "          cls.contains('chart-wrapper') ||",
        "          cls.contains('ib-preview-area'))",
        "      ) {",
...
def _hierarchy(stem: str, component: str) -> list[str]:
    ...
    return [
        ...
        "  const setting = ${setting};",
        *FORMAT_HELPER,
        "  const dimensionField = setting.dimensions.names[0];",
        "  const chartData = {",
        "    name: 'root',",
...
def _wordcloud() -> list[str]:
    return [
        ...
        "  const config = {",
        "    data,",
        "    wordField: setting.dimensions.names[0],",
...
def _rose() -> list[str]:
    return [
        ...
        *FORMAT_HELPER,
        ...
        "    xField: setting.dimensions.names[0],",
        "    yField: setting.measures[0],",
        "    seriesField: setting.dimensions.names[0],",
        "    ...(setting.color ? { color: setting.color } : {}),",
        "    meta: {",
        "      [measureField]: { alias: setting.labelsY || measureField, formatter: (v) => formatMeasure(v, measureField) }",
        "    }",
...
def build_code(stem: str, payload: dict) -> list[str]:
    ...
    if stem in {"grid_table"}:
        return _grid_table(stem, multi_dim=True)
    if family == "tiny":
...
def main() -> None:
    for path in sorted(CHARTS.glob("*.json")):
        ...
        # and shaped like the new template (has labelsX or dimensions.names prompt).
