...
public class RenameOperationHandler extends AbstractOperationsHandler {
     ...
     *
     * @param sourceArray 		 request parameter sourceArray
     * @return {@code true} if validated, otherwise {@code false}.
     */
    private boolean isSourceArrayValid(String sourceArray) {
        JsonArray sourceJSON;
        try {
            sourceJSON = JsonParser.parseString(sourceArray).getAsJsonArray();
     ...
     *
     * @param fileToBeRenamed 		 file to be renamed
    ...
    private void renameDirectory(File directoryToBeRenamed, String newName) {
        ...
        ApplicationUtilities.addLockPathForFileBrowser(resource);
        ...
        try {
            ...
            synchronized (RenameOperationHandler.class) {
                ...
                FileBrowserCacheUtils.deleteFromDb(resource);
                FileBrowserCacheUtils.madeChangesDb(directoryToBeRenamed.getAbsolutePath());
                ApplicationUtilities.removeLockPathForFileBrowser(resource);
            }
        } catch (Exception ex) {
            ApplicationUtilities.removeLockPathForFileBrowser(resource);
     ...
     * @param fileToBeRenamed 		 file to be renamed
     * @param newName         		 new name of the file
     * @param actualExtension 		 actual extension of the file to be renamed
     */
    protected void modifyXml(File fileToBeRenamed, String newName, String actualExtension) {
        ApplicationUtilities.addLockPathForFileBrowser(fileToBeRenamed.getAbsolutePath());
        ...
        } else if (JsonUtils.getReportExtension().equalsIgnoreCase(actualExtension) ||JsonUtils.getHrReportExtension().equalsIgnoreCase(actualExtension)) {
            AdhocReport adhocReport = JaxbUtils.unMarshal(AdhocReport.class, fileToBeRenamed);
            adhocReport.setReportName(newName);
            synchronizeFileOperation(fileToBeRenamed, adhocReport);
        } else if (JsonUtils.getMetadataExtension().equalsIgnoreCase(actualExtension)) {
            Metadata metadata = JaxbUtils.unMarshal(Metadata.class, fileToBeRenamed);
            metadata.setFileName(newName);
            synchronizeFileOperation(fileToBeRenamed, metadata);
        } else if (JsonUtils.getDesignerExtension().equalsIgnoreCase(actualExtension)) {
            handleDesignerRename(fileToBeRenamed, newName);
        ...
        } else if (JsonUtils.getDesignerExtendedExtension().equalsIgnoreCase(actualExtension)) {
            handleDesignerRename(fileToBeRenamed, newName);
        ...
        }else if (JsonUtils.getHCRExtension().equalsIgnoreCase(actualExtension)) {
            HCReport hcr = JaxbUtils.unMarshal(HCReport.class, fileToBeRenamed);
            hcr.setName(newName);
            synchronizeFileOperation(fileToBeRenamed, hcr);
        } else {
            ApplicationUtilities.removeLockPathForFileBrowser(fileToBeRenamed.getAbsolutePath());
            throw new OperationFailedException("The file " + fileToBeRenamed + " can't be renamed" +
                    " as only saved results , reports and designer are supported.");
        }
        ApplicationUtilities.removeLockPathForFileBrowser(fileToBeRenamed.getAbsolutePath());
    }
    /**
     * Handles the renaming of a Dashboard Designer file. It reads the contents of the specified
     * Dashboard Designer file, updates the name with the new name, and synchronizes the changes by performing a
     * file operation.
     *
     * @param fileToBeRenamed 		 Dashboard Designer file to be renamed.
     * @param newName         		 new name to be assigned to the Dashboard Designer.
     */
    private void handleDesignerRename(File fileToBeRenamed, String newName) {
        EfwDashboardDesigner efwDashboardDesigner = JaxbUtils.unMarshal(EfwDashboardDesigner.class,
                fileToBeRenamed);
        efwDashboardDesigner.setName(newName);
        synchronizeFileOperation(fileToBeRenamed, efwDashboardDesigner);
    }
    ...
    /**
     * Synchronizes the file operation for renaming a resource. It ensures that the file operation is performed
     * atomically by synchronizing on the class object.
     *
     * @param fileToBeRenamed 		 file representing the resource to be renamed.
     * @param iResource       		 resource object containing the updated information.
     */
    private void synchronizeFileOperation(File fileToBeRenamed, IResource iResource) {
        synchronized (RenameOperationHandler.class) {
            ...
            FileBrowserCacheUtils.madeChangesDb(fileToBeRenamed.getAbsolutePath());
        }
    }
}
