...
/**
 * The {@code AIAgentCreatorDb} class is an implementation of the {@link IComponent} interface, responsible for
 * creating or editing  Reports. It handles the execution of agent creation or editing based on
 * the provided JSON form data.
 *
 * @author Somen
 */
@SuppressWarnings("unused")
public class AIAgentCreatorDb implements IComponent {
	/**
     * Executes the agent reation or editing based on the provided JSON form data.
     *
     * @param jsonFormData 		 formData containing parameters(like location, state, uuid etc.) for agent creation or editing.
    ...
    public String executeComponent(String jsonFormData) {
        ...
        if (formDataJson.get("state").isJsonNull()) {
            throw new IncompleteFormDataException("The agent state parameter is null.");
        }
        ...
        String requiredExtension = "." + (JsonUtils.getAiAgentExtension());
        if (formDataJson.has("uuid")) {
            ...
            if(formDataJson.has("agentName")) {
                String agentName = formDataJson.get("agentName").getAsString();
                hiResource.setTitle(agentName);
            }

            
            instantAgentEdit(hiResource, createdDate, formDataJson, state, metadataResource);
            ...
            if (!formDataJson.has("agentName")) {
                throw new IllegalArgumentException("The parameter agentName is not present");
            }
            String agentName = formDataJson.get("agentName").getAsString();
            //agentName = DBProcessor.checkAndReplaceSpecialChars(agentName).trim();
            hiResource = serviceDB.getResourceByUrl(location + "/" + checkAndReplaceSpecialChars(agentName) + requiredExtension);
            if (agentName.length() < 3) {
                throw new EfwServiceException("At least 3 characters has to be provided for report name");
            }

            String newName = checkAndReplaceSpecialChars(agentName) + requiredExtension;
            if (hiResource != null) {
                newName = agentName + "_" + String.valueOf(System.currentTimeMillis()).substring(11, 13) + requiredExtension;
            }
            ...
            hiResource.setTitle(agentName);
            String resourcePath = DBProcessor.checkAndReplaceSpecialChars(agentName).trim();
            hiResource.setResourceType(resourceTypeServiceDB.getResourceTypeByTypeAndExtension("agent", ".agent"));
            saveAiAgent(hiResource, createdDate, formDataJson, state, metadataResource, createdBy, agentName);
        ...
        result.addProperty("message", "Successfully saved agent file");
    ...
    /**
     * Saves the Report and HIResource information during agent creation.
     *
     ...
     * @param agentName    The name of the report.
     */
    private void saveAiAgent(HIResource hiResource, Date createdDate, JsonObject formDataJson, String state, HIResource metadataResource, String createdBy, String agentName) {
        HIResourceAIAgent aiAgent = new HIResourceAIAgent();
        aiAgent.setCreatedDate(createdDate);
        aiAgent.setCreatedBy(Integer.valueOf(createdBy));
        aiAgent.setAgentName(agentName);
        coreSave(aiAgent, createdDate, formDataJson, state, metadataResource);
        hiResource.setAiAgent(aiAgent);
    }

    /**
     * Handles the report editing process during agent creation or editing.
     *
     ...
     * @param metadataResource The metadata resource associated with the report.
     */
    private void instantAgentEdit(HIResource hiResource, Date createdDate, JsonObject formDataJson, String state, HIResource metadataResource) {
        HIResourceAIAgent aiAgent = hiResource.getAiAgent();
        coreSave(aiAgent, createdDate, formDataJson, state, metadataResource);
    }
     ...
     *
     * @param hReport       	The HIResourceAIAgent instance to be saved or updated.
     * @param createdDate   	The date when the report is created or edited.
     * @param formDataJson  	The JSON data containing form parameters for the report.
     * @param state         	The state of the report.
     * @param metadataResource  The metadata resource associated with the report.
     */
    private void coreSave(HIResourceAIAgent hReport, Date createdDate, JsonObject formDataJson, String state, HIResource metadataResource) {
