...
var tableCount=0;
function genrateAlias(){
	return "table"+"_"+(tableCount++);
}
...
function setQueryOffsetLimit(query, offset, limit, context) {
    ...
    for (var ind = 0; ind < requestedColumnsArray.length; ind++) {
		...
		if(singleColumn.custom){
			customColumns.push(requestedColumnsWithFullyQualified);
		}
...
if(customColumns.length>0){
	for(cc=0;cc<customColumns.length; cc++){
		selectWithoutDbName = selectWithoutDbName.replace(customColumns[cc],"custom#_"+cc);
	}
}
    var fromToOrderBy;
    if (!orderByWithoutDbName)
	orderByWithoutDbName="order by\n\t"+ aliasesText[0]+" asc";
    if (orderByWithoutDbName){
		
var splitSelect = selectWithoutDbName.replace("select","");
...
var selectTokenized={};
		...
		do{
			 ...
			 asIndex = splitSelect.indexOf(" as ",start);
		 comaIndex = splitSelect.indexOf(",",asIndex);
			if(comaIndex==-1){
				comaIndex=splitSelect.length;
			}
			
			
			var requiredText=splitSelect.substring(start,comaIndex).trim();
			var keyValSplit=requiredText.split(" as ");
			selectTokenized[keyValSplit[1]]=keyValSplit[0];
			start=comaIndex+1;
		
		}while(comaIndex<splitSelect.length)
			orderByWithoutDbName=orderByWithoutDbName.replace("order by\n\t","");
		
			  var orderBySplit = orderByWithoutDbName.split(",")
				var orderByReq=[];
				for(i=0;i<orderBySplit.length;i++){
					var itm = orderBySplit[i]
					if(itm.indexOf("asc")>-1){
						itm = itm.replace("asc","").trim();
						orderByReq.push(selectTokenized[itm] + " asc");
					}else{
						itm = itm.replace("desc","").trim();
						orderByReq.push(selectTokenized[itm] + " desc");
					}
        ...
        fromToOrderBy = "order by " + orderByReq.join(",").replace("distinct","");
    ...
    if (applyDistinctOnOuter(obj) || checkDistinctOnFirstColumn(obj)) {
        ...
        aliasesJoined = "distinct  " + aliasesJoined;
		 appliedRank = "dense_rank()"
            ...
            requiredQueryForSpark = selectKeyword + '' + aliasesJoined + ' from (' + selectWithoutDbName + ',' + appliedRank + ' over ( ' + fromToOrderBy + ') as `rank`';
			
		if(customColumns.length>0){
		
						for(cc=0;cc<customColumns.length; cc++){
							requiredQueryForSpark = requiredQueryForSpark.replaceAll("custom#_"+cc,customColumns[cc]);
							selectWithoutDbName = selectWithoutDbName.replaceAll("custom#_"+cc,customColumns[cc]);
							fromToOrderBy = fromToOrderBy.replaceAll("custom#_"+cc,customColumns[cc]);
						}
	...
	var tableAndAlias = obj.tablesAlias;
	
		
	for (var indx = 0; indx < obj.tables.length; indx++) {
		
        var requestedTableForMap = obj.tables[indx];
		var aliasTable=tableAndAlias[requestedTableForMap]+"`";
		  var requestedTable= requestedTableForMap+"`"
		requiredQueryForSpark = requiredQueryForSpark.replaceAll(requestedTable,aliasTable);
	whereWithoutDbName = whereWithoutDbName.replaceAll(requestedTable,aliasTable);
	groupByWithoutDbName = groupByWithoutDbName.replaceAll(requestedTable,aliasTable);
	havingWithoutDbName = havingWithoutDbName.replaceAll(requestedTable,aliasTable);

		
    }

        requiredQueryForSpark += fromClause + whereWithoutDbName + groupByWithoutDbName + havingWithoutDbName + ' ) outerQuery '; requiredQueryForSpark += limit!=0?'where `rank` between ' + (parseInt(offset) + 1) + ' and ' + (parseInt(offset) + parseInt(limit) + '') : " order by 1";
    ...
    return actlQry.replaceAll("_#1_", "`.`")
	;
