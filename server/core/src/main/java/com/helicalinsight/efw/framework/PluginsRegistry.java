 ...
 * <p/>
 * Consists of two ConcurrentHashMaps one representing all available classes of
 * the plugins; and the other consisting of the class loaders for those
 * classes(entry points) which are used.
 *
 * @author Rajasekhar
 */
public enum PluginsRegistry {
    ...
    public PluginEntry registerPlugin(String entryPointClass, PluginEntry pluginEntry) {
        ...
        if(!plugin.getEntryPointClasses().contains(entryPointClass))
    	...
    	logger.debug("Afer setting: Reference Data "+plugin);
        PluginEntry old=this.enabledPlugins.putIfAbsent(entryPointClass, pluginEntry);
    	logger.debug("Existing entry for :"+entryPointClass+" is: "+old);
        if(old!=null)
        	return (old.getPlugin().getEntryPointClasses().contains(entryPointClass)?null:old);
        else
        	return null;
    }
    ...
    public void closeAllClassLoaders() {
        for (Map.Entry<String, PluginEntry> entry : this.enabledPlugins.entrySet()) {
            PluginEntry pluginEntry = entry.getValue();
            List<String> names = pluginEntry.getPlugin().getEntryPointClasses();
            for (String aClass : names) {
                deRegisterAndClosePlugin(aClass);
            }
