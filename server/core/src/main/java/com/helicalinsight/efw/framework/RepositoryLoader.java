 ...
 * Individual jar plugins are also opened and all the classes and their names
 * are stored.
 * <p/>
 *
 * @author Rajasekhar
...
class RepositoryLoader {
    ...
    private void load(File[] files) {
        ...
        for (File aFile : files) {
            if (aFile.isDirectory()) {
                directoryList.add(aFile);
            } else {
                fileList.add(aFile);
            }
        }

        processDirectory(directoryList);

        processFile(fileList);

    }

    public void processFile(List<File> fileList) {
        for (File aFile : fileList) {
            loadFile(aFile.getAbsolutePath());
        }
    }

    public void processDirectory(List<File> directoryList) {
        for (File aFile : directoryList) {
    ...
    private synchronized void loadFile(String absolutePath) {
    ...
    public synchronized void makeEntry(File file) {
        ...
        if (file.isDirectory()) {
            if (plugin != null) {
                ...
                // A folder is a valid plugin folder only when it is a direct
                // descendant of Plugins or Drivers.
                ...
                if (isValidPluginFolder(absolutePath)) {
                    ...
                    REPOSITORY.update(absolutePath, plugin);
                    // As only one CREATE event is observed when a folder is
                    // copied to Plugins/Drivers
            ...
            // Logically getParent() should give you a directory, which is the
            // name of the plugin folder or the
            ...
            if (isWatchedDirectory(directory)) {
                ...
                // Already a plugin is available. So place this jar/file inside
                // the plugin
                plugin = REPOSITORY.getPlugin(directory);

                if (plugin == null) {
                    logger.error(String.format(
                            "Expecting a plugin directory in the repository but found null. " + "Can't add %s",
                            absolutePath));
                } else {
    ...
    private boolean isValidPluginFolder(String absolutePath) {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("The file " + parent
                    + (watchedDirectory ? " is a watched resource" : " is not a watched " + "resource."));
        }
    ...
    public synchronized void deleteEntry(File file) {
        ...
        if (file.isDirectory() || (plugin != null && plugin.getDirectory() != null)) {
            if (plugin != null) {
                ...
                logger.debug(String.format("Deleted a folder %s from repository.", deletedPlugin.getName()));
            } else {
                logger.debug(String.format("Unknown event. Can't delete a non existing folder %s from repository.",
                        absolutePath));
            }
        } else {
            String directory = file.getParent();
