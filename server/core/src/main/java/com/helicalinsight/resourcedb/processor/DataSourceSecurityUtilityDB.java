...
public class DataSourceSecurityUtilityDB {
    ...
    private static final Logger logger = LoggerFactory.getLogger(DataSourceSecurityUtilityDB.class);
    ...
    public static void checkEfwdPermission(String id, File efwdFile, String operation) {
        ...
        try {
            ruleInstance.validateFile(fileAsJson);
...
/*
    ...
    public static void isDataSourceAuthenticated(JSONObject formJson) {
        ...
        if (formJson.has("metadataFileJson") && GlobalJdbcTypeUtils.isManagedGroovyDataSource(type)) {
        ...
        if (!dir.isEmpty()) {
            ...
            File efwdFile = null;
            if (fileName != null)
                efwdFile = new File(actualPath + File.separator + fileName);
            else
                efwdFile = ApplicationUtilities.getEfwdFile(dir);
