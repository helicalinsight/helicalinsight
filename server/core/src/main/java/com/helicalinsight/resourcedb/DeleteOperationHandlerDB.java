...
@Component
@Scope("prototype")
public class DeleteOperationHandlerDB extends AbstractOperationsHandler {

    private static final Logger logger = LoggerFactory.getLogger(DeleteOperationHandlerDB.class);
    ...
    private boolean canDeleteDirectories() throws UnSupportedRuleImplementationException {
        DeleteOperationUtilityDB deleteOperationUtility = new DeleteOperationUtilityDB();
    ...
    public Boolean deleteFolder(Integer id){
        HIResource hiResourceFolderByResourceId = hiResourceServiceDB.getHIResourceById(id);
        if(null!=hiResourceFolderByResourceId){
            try {
                hiResourceServiceDB.deleteHIResourceById(id);
                return true;
            } catch (Exception e) {
                e.printStackTrace();
                return false;
